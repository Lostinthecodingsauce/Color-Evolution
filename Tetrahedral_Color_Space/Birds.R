...
coldist <- vis_model%>%
  ...
  mutate(model = purrr::map(data, ~
                              (as.character(.x$value)%>%
                                 as.numeric()%>%
                                 coldist(.,   noise = "neural", achromatic = TRUE, n = c(1, 2, 2, 4),
                                         weber = 0.1, weber.achro = 0.1))$colddist%>%
                                 as.data.frame()))%>%
           dplyr::select(-data)%>%
           unnest(model)

### 
yeet <-read.csv("./second/FUCK.csv")
rownames(yeet) <- yeet[,1,]
yeet<- yeet[,c(2,3,4,5)]

coldist(vis_model, noise = "quantum", achromatic = TRUE, n = c(1,2,3,4), weber = 0.1, weber.achro = 0.1)
