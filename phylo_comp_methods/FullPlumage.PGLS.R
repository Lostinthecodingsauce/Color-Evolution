...
#####
#This <- read.csv("Alldata.csv", header = TRUE, fileEncoding = 'UTF-8-BOM') # without Cyanocompsa cyanoides
data <- read.csv("Full.csv", header = TRUE, fileEncoding = 'UTF-8-BOM') #No individuals removed 
...
#comp.data<-comparative.data(Card.tree, what, names.col = "ï..Species", vcv.dim=2, warn.dropped=TRUE)
###########
...
null=pgls(fPC1~mPC1, data=comp.data, lambda="ML")
...
#Forest
a2<-pgls(mPC1~Forest.Dependency, data=comp.data,lambda = "ML") ### Not
b2<-pgls(mPC2~Forest.Dependency, data=comp.data,lambda = "ML")## Medium
c2<-pgls(fPC1~Forest.Dependency, data=comp.data,lambda = "ML") ## Not
d2<-pgls(fPC2~Forest.Dependency, data=comp.data,lambda = "ML")## Not
summary(a2)
summary(b2)
summary(c2)
summary(d2)
...
#Habitat
a3<-pgls(mPC1~Habitat, data=comp.data,lambda = "ML") ### Not  ### @nd best model selection 
b3<-pgls(mPC2~Habitat, data=comp.data,lambda  = "ML")## Not
c3<-pgls(fPC1~Habitat, data=comp.data,lambda = "ML") ## Not
d3<-pgls(fPC2~Habitat, data=comp.data,lambda = "ML")## Not
summary(a3)
summary(b3)
summary(c3)
summary(d3)
...
## Strata  
a4a<-pgls(mPC1~StrataA, data=comp.data,lambda = "ML")## Under** sig ## 3rd best model selection 
a4<-pgls(mPC1~Strata, data=comp.data,lambda = "ML")## Under* sig
b4a<-pgls(mPC2~StrataA, data=comp.data,lambda = "ML")## None
b4<-pgls(mPC2~Strata, data=comp.data,lambda = "ML")## None
c4a<-pgls(fPC1~StrataA, data=comp.data,lambda = "ML")## None
c4<-pgls(fPC1~Strata, data=comp.data,lambda = "ML")## None
d4a<-pgls(fPC2~StrataA, data=comp.data,lambda = "ML")## None
d4<-pgls(fPC2~Strata, data=comp.data,lambda = "ML")## Under*  U/C = 0.054
summary(a4)
summary(b4)
summary(c4)
summary(d4)
...
a5a<-pgls(mPC1~StrataA+Migration, data=comp.data,lambda = "ML")## Under sig**
a5<-pgls(mPC1~Strata+Migration, data=comp.data,lambda = "ML")## None
b5a<-pgls(mPC2~StrataA+Migration, data=comp.data,lambda = "ML")## None
b5<-pgls(mPC2~Strata+Migration, data=comp.data,lambda = "ML")## None
c5a<-pgls(fPC1~StrataA+Migration, data=comp.data,lambda = "ML")## Understory* +Migration** 
c5<-pgls(fPC1~Strata+Migration, data=comp.data,lambda = "ML")## None
d5a<-pgls(fPC2~StrataA+Migration, data=comp.data,lambda = "ML")## U/C* Migration*
d5<-pgls(fPC2~Strata+Migration, data=comp.data,lambda = "ML")## Understory** U/C*, Migration*
summary(a5a)
summary(a5)
## Strata plus Forest
a6a<-pgls(mPC1~StrataA+Forest.Dependency, data=comp.data,lambda = "ML")## Under** highly sig ### Best model selection
a6<-pgls(mPC1~Strata+Forest.Dependency, data=comp.data,lambda = "ML")## Under highly sig
b6a<-pgls(mPC2~StrataA+Forest.Dependency, data=comp.data,lambda = "ML")## Medium sig
b6<-pgls(mPC2~Strata+Forest.Dependency, data=comp.data,lambda = "ML")## Medium**, no forest*
c6a<-pgls(fPC1~StrataA+Forest.Dependency, data=comp.data,lambda = "ML")## not 
c6<-pgls(fPC1~Strata+Forest.Dependency, data=comp.data,lambda = "ML")## Not
d6a<-pgls(fPC2~StrataA+Forest.Dependency, data=comp.data,lambda = "ML")## Not
d6<-pgls(fPC2~Strata+Forest.Dependency, data=comp.data,lambda = "ML")## not
summary(a6a)
summary(b6a)
summary(b6a)
summary(d6a)
...
a7a<-pgls(mPC1~StrataA+Habitat, data=comp.data,kappa = "ML")## terrestrial*, U/C, open habitat**
a7<-pgls(mPC1~Strata+Habitat, data=comp.data,delta = "ML")## Singular
b7a<-pgls(mPC2~StrataA+Habitat, data=comp.data,lambda = "ML")## None
b7<-pgls(mPC2~Strata+Habitat, data=comp.data,lambda = "ML")## Singular
c7a<-pgls(fPC1~StrataA+Habitat, data=comp.data,lambda = "ML")## none 
c7<-pgls(fPC1~Strata+Habitat, data=comp.data,lambda = "ML")## Singular
d7a<-pgls(fPC2~StrataA+Habitat, data=comp.data,lambda = "ML")## not
d7<-pgls(fPC2~Strata+Habitat, data=comp.data,lambda = "ML")## Singular
summary(a7a)

summary(a7a)

# Habitat + Migration 
a8<-pgls(mPC1~Habitat+Migration, data=comp.data,lambda = "ML") ### no
b8<-pgls(mPC2~Habitat+Migration, data=comp.data,lambda = "ML")## no  #### Best model selection 
c8<-pgls(fPC1~Habitat+Migration, data=comp.data,lambda = "ML") ## no
d8<-pgls(fPC2~Habitat+Migration, data=comp.data,lambda = "ML")## no
summary(c8)
...
# Give them names
names(fPC1) <- names(mPC1) <- rownames(Butt)

# Calculate PICs
hPic <- pic(mPC1, Card.tree)
aPic <- pic(fPC1, Card.tree)
...
PC1_lamda <-gls(fPC1~mPC1, data=Butt, correlation=Pagel, method = "ML")
...
# PC2
PC2_lamda <-gls(fPC2~mPC2, data=Butt, correlation=Pagel)
summary(PC2_lamda)
coef(PC2_lamda)
plot(fPC2 ~ mPC2)
...
###### Plotting 
M1_F1 <-gls(mPC1~fPC1, data=Butt, correlation=Pagel,method = "ML")
plot(mPC1~fPC1, data=Butt, correlation=Pagel,method = "ML")
abline(M1_F1)

M2_F2 <-gls(mPC2~fPC2, data=Butt, correlation=Pagel,method = "ML")
summary(M2_F2)
plot(fPC2~mPC2, data = comp.data,correlation=Pagel,method = "ML")
abline(M2_F2)
...
# Code to combine all outputs and AIC values
# http://www.mpcm-evolution.com/practice/online-practical-material-chapter-12/chapter-12-1-evolutionary-models-different-combinations-predictors
pred.vars = c("Forest.Dependency", "StrataA", "FUCK", 
              "Migration")
...
comp.data<-comparative.data(FullCard.tree, This, names.col = "species", vcv=TRUE, warn.dropped=TRUE)
coefs=m.mat# define an object to store the coefficients   
for (k in 1:length(models)) {
  res = try(pgls(as.formula(models[k]), data = comp.data, lambda = "ML"))
